Shader "Clouds Shader"
{
        ...
        Subshader
    {
        ...
        Pass
        {
            ...
            float _octaves,_lacunarity,_gain,_value,_amplitude,_frequency, _offsetX, _offsetY, _power, _scale, _cutoff, _monochromatic, _range;
            float4 _color;
            ...
            float4 pixel_shader(SHADERDATA ps) : SV_TARGET
            {
                ...
                if (_monochromatic == 0.0)
                    return float4(0,0,0,(1 - c) * _cutoff);
                else
                if (c < _range)
                    return 0;
                else
